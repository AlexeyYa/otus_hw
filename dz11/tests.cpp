...
TEST(LIBASYNC, Recieve) {
    ...
    // Act
    char src[4] = "abc";
    async::receive(handle, src, 1);
    async::receive(handle, src + 1, 1);
    async::receive(handle, src + 2, 1);
    // Waiting for output
...
TEST(LIBASYNC, RecieveMultiple) {
    ...
    char src1[4] = "abc";
    char src2[4] = "xyz";
    async::receive(handle1, src1, 1);
    async::receive(handle2, src2, 1);
    async::receive(handle1, src1 + 1, 1);
    async::receive(handle2, src2 + 1, 1);
    async::receive(handle1, src1 + 2, 1);
    async::receive(handle2, src2 + 2, 1);
    // Waiting for output
